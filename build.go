	...
	go func() {
		for {
			...
			if _, ok := pushImageReqMap[imageName]; !ok {
				...
				go func() {
					for pushImgReq := range pushImageReqMap[imageName] {
...
func cloneGit(buildSpec *BuildSpec, projectDir string, outputStream io.Writer) error {
	...
	repo, resp, err := githubClient.Repositories.Get(ctx, buildSpec.GithubOrgName, buildSpec.GithubRepoName)
	if err != nil {
		return err
	}
...
func updateSubmodules(projectDir, githubHost, githubToken string, outputStream io.Writer) error {
	...
	submoduleStrs := strings.Split(string(bytes.TrimSpace(submoduleBytes)), "\n")
...
//--------------------------------------------------------------
// Exported functions for Go routines

func BuildKubernetes() {
	println("build")
}

func BuildDocker(buildSpec *BuildSpec) error {
	buildResult := &BuildResult{
